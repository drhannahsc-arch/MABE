"""
MABE Sprint 35b: Thermodynamic Calibration v2
8 physics fixes validated against 33 experimental formation constants.
R²=0.24, MAE=5.8, Bias=+0.7. Mixed class MAE=4.5.
"""
import os
...
print("\n\U0001f528 MABE Sprint 35b v2 \u2014 Thermodynamic Calibration\n")
...
def compute_enhanced_thermodynamics(recognition, structure, interior, problem):
    ...
    for da in donors:
        ...
        # Empirical per-donor exchange energy: ΔG for replacing one water
        # with one donor atom in the inner coordination sphere.
        # Values derived from average stepwise log K₁ values (kJ/mol):
        #   O-donor (carboxylate): ~5-10 kJ/mol for divalent
        #   N-donor (amine/pyridine): ~10-18 kJ/mol for divalent
        #   S-donor (thiolate): ~15-30 kJ/mol for soft metals
        # Charge scaling: roughly linear with charge (trivalent ~1.5× divalent)
        base_exchange = {"O": -7.0, "N": -9.0, "S": -21.0, "P": -18.0,
                         "Cl": -5.0, "Br": -8.0, "I": -15.0}
        dg_exchange = base_exchange.get(da, -8.0)

        # Charge scaling: electrostatic field scales as z²/r for binding strength
        # +1 → weak, +2 → moderate, +3 → strong
        charge_factor = -5.0 * (charge**2 - 1) / max(1, len(donors))
        dg_exchange += charge_factor

        # HSAB match bonus/penalty: the primary selectivity driver.
        # Well-matched pairs gain extra stabilization.
        # Mismatched pairs (hard metal + soft donor) are strongly penalized.
        mismatch = abs(softness - ds)
        # Match bonus: up to -10 kJ/mol for perfect match
        match_bonus = -10.0 * (1.0 - mismatch)
        dg_exchange += match_bonus * max(softness, ds)
        # Mismatch penalty: hard metal + soft donor or vice versa
        # Ca2+(0.01) + S(0.80): mismatch=0.79 → penalty = +15 kJ/mol per donor
        # This is the key driver of HSAB selectivity
        if mismatch > 0.3:
            dg_exchange += 20.0 * (mismatch - 0.3)  # Strong penalty for mismatch
    ...
    if d_electrons > 0 and d_electrons < 10:
        try:
            ...
            # ΔLFSE: difference between ligand complex and aqua complex
            # The aqua complex also has LFSE, so the GAIN from ligand substitution
            # is only the difference, not the full LFSE.
            ...
            # Cap ΔLFSE at physically reasonable range.
            # Maximum LFSE DIFFERENCE (ligand vs water) is ~80 kJ/mol
            # (strong-field d8 square planar, e.g. Ni-CN4 vs Ni-H2O6)
            dg_lfse = max(-80.0, min(0.0, dg_lfse))
            # Negative if ligand LFSE > water LFSE (stronger field = more stabilization)
    ...
    dg_net = (dg_bind + dg_desolv + dg_preorg + dg_chelate + dg_electrostatic
              ...
              + dg_relativistic + dg_translational)
    ...
    for da in donor_atoms:
        ...
        if key in _COVALENT_BONDS:
            ...
            if bde >= _COVALENT_THRESHOLD:
                ...
                coord_fraction = _COVALENT_FRACTIONS.get(metal_formula, 0.12)
...
print("\n\u2705 Sprint 35b v2 calibration files created!\n")